...
import { createClient } from "@/lib/clients/supabase-client-browser";
...
type DatabaseSchema = DatabaseType["public"];
...
type UseInfiniteQueryProps<T extends SupabaseTableName> = {
...
>(props: UseInfiniteQueryProps<T>) {
  ...
  // Use a ref to track previous props for comparison
  const prevPropsRef = useRef(props);

  useEffect(() => {
    const prevProps = prevPropsRef.current;
    const propsChanged =
      prevProps.tableName !== props.tableName ||
      prevProps.columns !== props.columns ||
      prevProps.pageSize !== props.pageSize ||
      prevProps.trailingQuery !== props.trailingQuery;
